...
- 面向 `.lig` 源文件的词法器、解析器、类型检查器、求值器、格式化器、文档生成器和 C 后端。
- 基于 `ligare.toml` 的包模式，支持 new/init、build、update、test、fmt 和 doc 命令。
...
| `new [--bin\|--lib] PATH` | 创建新的包目录，包含 `ligare.toml`、`src` 入口文件和 Git 脚手架。 |
| `init [--bin\|--lib] [PATH]` | 在当前目录或目标目录中初始化包，保留现有文件并补充 Git 脚手架。 |
...
| `doc [--private] [-o PATH] [PATH]` | 从 `.lig` 文件生成 Markdown 文档。 |
...
crates/ligare_fmt 格式化 crate
crates/ligare_doc Markdown 文档生成 crate
...
- `Point{...}` 支持按字段名初始化；当上下文已经给出目标结构体类型时，也可以直接写 `{...}`。
...
def y  : Option int     := Option::None              -- 需要类型标注来推断类型参数
...
def j : PosOption := PosOption::Just 5       -- 自动证明：5 > 0
def k : PosOption := PosOption::Just (-3)    -- 编译错误：-3 > 0 不成立
```
...
match opt with
| Option::None =>
  -- 此处自动获得 theorem：opt = Option::None
| Option::Some val =>
  -- 此处自动获得 theorem：opt = Option::Some val
...
| `#[instance]` + `def <名称> : <类型> := <体>` | 注册一个隐式编译期实例 |
| `theorem <名称> : <类型> := <体>` | 定义命名定理（经类型检查后可作为项使用） |
| `#check <表达式> : <类型>` | 对表达式进行类型检查 |
| `#eval <表达式>` | 求值表达式并显示结果 |

隐式实例通过给普通定义加上 `#[instance]` 属性来声明：
```ligare
#[instance]
